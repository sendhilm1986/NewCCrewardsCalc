/**
 * WordPress Functions for Secure Calculator Integration
 * Add this to your theme's functions.php or use a code snippets plugin
 */

// PHP code to add to functions.php (as JavaScript comment for reference)
/*
<?php
...
// JavaScript nonce setup for WordPress
document.addEventListener('DOMContentLoaded', function() {
    // Set up nonce for AJAX requests
    if (typeof ccalc_ajax !== 'undefined') {
        window.ccalc_nonce = ccalc_ajax.nonce;
    } else {
        // Fallback: try to get nonce from WordPress
        const nonceElement = document.querySelector('meta[name="ccalc-nonce"]');
        if (nonceElement) {
            window.ccalc_nonce = nonceElement.getAttribute('content');
        }
    }
});